...
当前基线包含 60 项远程服务测试和 16 项 TCP 工具测试。Build 14 已通过无签名真机目标编译；最近一次签名安装证据是 Build 13，自动启动因设备锁屏被系统拒绝，最近一次完整系统启动证据仍是 Build 12。代码存在或 App 能启动不等于所有硬件能力均已实车验收，具体状态见 [实现状态](iot-bluetooth/IMPLEMENTATION-STATUS.md)。
