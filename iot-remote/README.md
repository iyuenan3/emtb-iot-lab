...
2026 年 8 月 22 日部署基线为 Git revision `05043e01122062eba33a45a8b7b811eed04e1097`。发布目录和线上目录分别通过远端 Python 3.12 的 72 项测试，五个核心源码哈希一致；主服务切换前停止为 `Result=success`，内部与公网健康接口读回相同 revision 且设备在线，未认证 API 为 401，主页为 200。`emtb-iot-backup.timer` 已启用并保持活动，首次任务成功，恢复目录只有三个 600 权限的托管文件且没有明文副本；服务器只保存 644 权限公钥证书，配置目录和恢复目录均为 700。真实服务器密文已下载到持有钥匙串恢复身份的 Mac，完成摘要、解密、18 张表和 SQLite 完整性恢复演练，临时明文随后清理。部署前备份位于 `backups/20260822-041250-pre-05043e0`，只读发布目录位于 `releases/05043e01122062eba33a45a8b7b811eed04e1097`。真实 BLE 事件队列、W0、D1、告警定位链、60 秒轨迹和离线移动阈值仍待实车验收。
