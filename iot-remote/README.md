...
> 历史模块：线上服务已于 2026 年 8 月 23 日停止。服务器上的程序、数据、脚本、发布目录、旧名目录和历史备份均已永久删除，公网路由也已移除。本目录只供代码审计和协议研究，不代表当前存在部署；没有新的明确授权时不得恢复。

本目录保存此前单车专用 IoT TCP 接入和 iPhone HTTP API 的实现。服务不接入原业务数据库，也不依赖第三方 Python 包。
...
2026 年 8 月 23 日最后一次部署基线为 Git revision `469f154b75bb243e61cd1dd3f291fde0256bb7bb`。只含 Git 跟踪文件的发布归档 SHA-256 为 `95fa557d9520865d71d02ae15eb96cb9188434bedbe7a881a9764f296496aba3`；只读发布目录和线上目录分别通过远端 Python 3.12 的 77 项测试，19 个跟踪文件逐项哈希一致。主服务切换前停止为 `Result=success`，systemd 解析读回 1 分钟内最多重启 5 次。部署验收发现用户级 systemd 原为 `Linger=no`，导致服务随 SSH 会话退出并制造间歇 502 与设备短连；启用 linger 后，全新 SSH 会话读回同一服务 PID，设备自动恢复在线，公网连续读回相同 revision，未认证 API 为 401，主页为 200。

最后一次手动加密副本为 `Result=success` 与退出码 0，恢复目录当时只有三个 600 权限的托管文件且没有明文。真实服务器密文曾下载到持有恢复身份的 Mac，完成元数据摘要、CMS 解密、18 张表和 SQLite 完整性校验，未覆盖线上数据库，临时密文与明文随后清理。此后按照用户授权永久删除远端 14 个精确目录内的全部项目资产，包括旧名 `qjqx`、历史备份、只读发布目录、线上数据、程序和脚本，并确认精确目标剩余数量为 0。真实 BLE 事件队列、W0、D1、告警定位链、60 秒轨迹和离线移动阈值在退役前未完成实车验收，现已停止推进。
