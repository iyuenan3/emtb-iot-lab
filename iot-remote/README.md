...
2026 年 8 月 22 日部署基线为 Git revision `5d0ef70`。`emtb-iot-remote.service` 已通过远端 Python 3.12 的 60 项测试、内部 readiness、公网 revision、未认证 API 401、主页 200、四个核心源码哈希、离线基线与告警比较数据库迁移、两个 unit 阈值及权限检查。设备在新版本上完成两次自动重连，真实活动 IoT 长连接下停止仍为 `Result=success`。旧源码、unit 和 SQLite 在线备份均保留为回滚材料。真实 W0、D1、告警定位链、60 秒轨迹和离线移动阈值仍待实车验收。
