...
2026 年 8 月 22 日部署基线为 Git revision `3d295e4`。`emtb-iot-remote.service` 已通过远端 Python 3.12 的 38 项测试、内部 readiness、公网 revision、未认证 API 401、主页 200、源码哈希、数据库迁移与权限检查。部署时旧 revision 在活动 IoT 长连接下再次出现已知停止超时；新 revision 随后在真实设备长连接下完成 `Result=success` 停止并恢复运行，证明显式关闭顺序生效。设备在首个 H0 前进入停机验收，因此 D1 期望值与确认值仍为空，D1 实车回包继续待验收。旧源码、unit 和 SQLite 在线备份均保留为回滚材料。
