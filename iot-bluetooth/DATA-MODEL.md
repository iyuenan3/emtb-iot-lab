...
Build 9 当前源码先实现精简的 `locations` 表，保存单次或追踪来源、设备时间、接收时间、有效性、WGS84 经纬度、卫星数、HDOP、海拔、模式和原始字段。相同设备、设备时间和原始字段使用 SHA-256 指纹去重。骑行、告警归属、漂移判定和展示拒绝原因仍按下表目标模型补充。
...
首版展示规则为：定位必须有效、经纬度范围合法、时间可解释，并通过可配置的 HDOP、瞬时位移和速度上限检查。阈值需通过户外样本确定，不写死为厂商未验证值。被过滤点仍保留原始记录。
