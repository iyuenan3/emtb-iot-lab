...
当前 `locations` 表保存单次或追踪来源、设备时间、接收时间、有效性、WGS84 经纬度、卫星数、HDOP、海拔、模式和原始字段。相同设备、设备时间和原始字段使用 SHA-256 指纹去重。质量管线已经补充 `display_eligible`、`rejection_reason`、相邻距离和估算速度，拒绝无效、时间缺失或超前、卫星不足、HDOP 过高、乱序和异常速度点。被拒绝点保留原始记录，但不替换地图可靠位置。骑行和告警归属仍按下表目标模型补充。
