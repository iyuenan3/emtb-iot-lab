...
Build 16 已落地 `audit_logs`，并覆盖配对、命令创建与状态、定位接收、骑行起止、告警事件、BLE 观察与操作事件、设置修改、设备会话和每日清理。API 同时返回结构化详情与安全文本摘要，详情不写入定位原始帧、原始地址或任何认证材料。
...
revision `05043e0` 已启用每日 timer 并成功生成首份线上密文。部署验收从服务器下载该密文，在持有恢复身份的 Mac 上完成钥匙串解密、归档校验、18 张表和 SQLite 完整性检查，未替换线上数据库，临时明文随后清理。
