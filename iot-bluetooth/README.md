...
这是一个面向自有车辆的最小化 iPhone App。Build 21 只保留附近蓝牙连接、开锁和关锁，不访问网络，不读取或推断车辆锁态，也不包含远程控制和维护功能。

## 当前行为

1. 用户手动连接指定车辆 IoT。
2. App 只完成 BLE 服务发现和设备密钥认证，不自动读取锁态、电量、骑行或系统信息。
3. 用户长按开锁或关锁 1.2 秒，再通过 Face ID 或设备密码。
4. 每次连接只允许一个控制动作。
...
源码守卫检查最小命令白名单、单连接单动作、必要回执后断开、设备所有者验证，以及远程代码未进入构建目标。编译通过不代表真实车辆动作已验收。

## 实车验收边界

Build 21 尚未安装，也尚未执行真实 BLE 控制。首次验收必须分步进行：先只连接和断开，确认仪表不重启；再单独验证一次开锁；最后从新的连接单独验证一次关锁。任一步出现仪表重启、状态反向或物理状态不一致，应立即关闭 App 蓝牙并停止测试。
