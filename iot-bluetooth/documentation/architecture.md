...
| 层级 | 代码 | 职责 | 当前状态 |
| --- | --- | --- | --- |
| 界面 | `ContentView.swift` | 车辆、地图、记录、更多四页导航 | Build 10 已实现 |
...
| 远程服务 | `../iot-remote/` | 单车 TCP、HTTPS API、命令状态机 | 已部署 |
| 位置存储 | `../iot-remote/iot_remote/database.py` | D0 有效与无效报告、去重和历史查询 | Build 9 本地已实现，待部署 |
...
- 地图和基础位置数据已实现。漂移过滤、骑行分段、保留清理、布防和告警仍未实现。
